...
def get_mongodb_collection():
    ...
    try:
        ...
        print(
            f"Successfully connected to MongoDB Atlas. DB: '{DB_NAME}', Collection: '{COLLECTION_NAME}'"
        )
        return collection
    except Exception as e:
...
def insert_data_to_mongodb(collection, texts: List[str], embeddings: List[List[float]]):
    ...
    for i in range(len(texts)):
        documents.append(
            {"text": texts[i], "embedding": embeddings[i], "source_doc": pdf_path}
        )
    ...
    print(
        f"Inserting {len(documents)} documents into MongoDB collection '{collection.name}'..."
    )
    try:
...
def make_rag_prompt(query: str, relevant_passages: List[str]) -> str:
    ...
    escaped_passage = (
        combined_passage.replace("'", "\\'").replace('"', '\\"').replace("\n", " ")
    )
...
def run_interactive_qa(collection):
    ...
    while True:
        ...
        relevant_passages = get_relevant_passages_mongodb(
            query, collection, n_results=3
        )
...
if __name__ == "__main__":
    ...
    if not check_data_exists(collection):
        ...
        if embeddings:
            ...
            print(
                "Data preparation complete. Make sure the Vector Search Index is 'Active' in Atlas before proceeding."
            )
            # time.sleep(180)
        else:
        ...
        print(
            "Data already exists in MongoDB collection. Skipping embedding generation and insertion."
        )
